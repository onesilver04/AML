...
DEFAULT_OUTPUT = PROJECT_ROOT / "SHAP/Condition2/Results/condition2_selected_samples.json"
DEFAULT_OUTPUT_DIR = (
    PROJECT_ROOT / "SHAP/Condition2/Results/condition2_selected_samples_individual_json"
)
...
CONDITION_2_SAMPLE_INDICES = {
    12, 18, 20, 21, 26, 66, 67, 104, 113, 150,
    158, 163, 165, 174, 180, 196, 199, 22, 64, 4,
    33, 90, 145, 116, 138, 152, 1, 54, 23, 2,
    193, 59, 0, 68, 162, 99, 27, 175, 70, 121,
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build selected sample JSON files for Condition2 from mixed RAG/QA/Answers folders."
    )
...
def load_shap_top3_by_sample(shap_dir: Path) -> dict[int, list[dict[str, Any]]]:
    ...
    for path in sorted(shap_dir.glob("shap_tuples_non_prefix_*.json")):
        ...
        if sample_idx not in CONDITION_2_SAMPLE_INDICES:
            continue
...
def build_sample_payload(
    ...
    if sample_idx not in CONDITION_2_SAMPLE_INDICES:
        raise ValueError(f"sample_idx={sample_idx} is not a Condition2 sample.")
...
def main() -> None:
    ...
    condition2_rows = [
        ...
        if int(row["sample_idx"]) in CONDITION_2_SAMPLE_INDICES
    ]

    if len(condition2_rows) != 40:
    ...
    missing_rag_files = sorted(
        sample_idx
        for sample_idx in CONDITION_2_SAMPLE_INDICES
        if sample_idx not in rag_payloads_by_sample
    )

    if missing_rag_files:
        print(
            "WARNING: Missing RAG final summary JSON files for Condition2 sample_idx: "
    ...
    if missing_korean_explanations:
        print(
            "WARNING: Missing Korean final explanation for Condition2 sample_idx: "
    ...
    if missing_shap_files:
        raise ValueError(
            "Missing all-dataset SHAP JSON files for Condition2 sample_idx: "
    ...
    print(f"Selected Condition2 samples : {len(payloads)}")
