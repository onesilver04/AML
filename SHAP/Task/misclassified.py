...
DEFAULT_OUTPUT = PROJECT_ROOT / "SHAP/Task/misclassified_test_samples.csv"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract every misclassified sample from the saved test set."
    )
...
def main():
    ...
    required_prediction_cols = {"sample_idx", "true_class", "predicted_class", "is_correct"}
    ...
    misclassified = predictions[predictions["is_correct"] == False].copy()
    misclassified = misclassified.merge(y_test, on="sample_idx", how="left")

    if misclassified["saved_true_class"].isna().any():
        missing = misclassified.loc[
            misclassified["saved_true_class"].isna(), "sample_idx"
        ].tolist()
        raise ValueError(f"Some sample_idx values were not found in y_test: {missing}")

    output = resolve_path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    misclassified.to_csv(output, index=False)

    total = len(predictions)
    count = len(misclassified)
    rate = count / total * 100 if total else 0.0
    sample_indices = misclassified["sample_idx"].astype(int).tolist()

    print(f"Total test samples: {total}")
    print(f"Misclassified samples: {count} ({rate:.2f}%)")
    print(f"Misclassified sample_idx: {sample_indices}")
