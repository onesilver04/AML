...
OUTPUT_DIR = Path("SHAP/Feature Importance")
AMBIGUOUS_LOW = 0.40
AMBIGUOUS_HIGH = 0.60
...
def build_case_report(y_true, y_pred, y_proba):
    report_df = pd.DataFrame(
        {
            "sample_idx": np.arange(len(y_true)),
            "y_true": np.asarray(y_true),
            "y_pred": np.asarray(y_pred),
            "bad_probability": np.asarray(y_proba),
        }
    )
    report_df["pred_label"] = np.where(
        report_df["y_pred"] == 1, "BAD CREDIT RISK", "GOOD CREDIT RISK"
    )
    report_df["true_label"] = np.where(
        report_df["y_true"] == 1, "BAD CREDIT RISK", "GOOD CREDIT RISK"
    )
    report_df["confidence_margin"] = (report_df["bad_probability"] - 0.5).abs()
    report_df["is_misclassified"] = report_df["y_true"] != report_df["y_pred"]
    report_df["is_ambiguous"] = report_df["bad_probability"].between(
        AMBIGUOUS_LOW, AMBIGUOUS_HIGH, inclusive="both"
    )
    report_df["case_type"] = np.select(
        [
            report_df["is_misclassified"] & report_df["is_ambiguous"],
            report_df["is_misclassified"],
            report_df["is_ambiguous"],
        ],
        [
            "misclassified_ambiguous",
            "misclassified",
...
case_report_df = build_case_report(y_test, pred_sel, proba_sel)
interesting_cases_df = case_report_df.loc[
    case_report_df["is_misclassified"] | case_report_df["is_ambiguous"]
].copy()

sample_indices = interesting_cases_df["sample_idx"].tolist()

if not sample_indices:
    sample_indices = (
        case_report_df.sort_values("confidence_margin", ascending=True)
        .head(MAX_SAMPLES_PER_GROUP)["sample_idx"]
        .tolist()
    )
    print(
        "\n애매한 confidence 또는 오분류 샘플이 없어 "
        "confidence가 가장 낮은 샘플들로 대체합니다."
    )

misclassified_cases_df = (
    case_report_df.loc[case_report_df["is_misclassified"]]
    .sort_values(["confidence_margin", "bad_probability"], ascending=[True, False])
    .head(MAX_SAMPLES_PER_GROUP)
    .copy()
)
ambiguous_cases_df = (
    case_report_df.loc[case_report_df["is_ambiguous"]]
    .sort_values(["confidence_margin", "bad_probability"], ascending=[True, False])
    .head(MAX_SAMPLES_PER_GROUP)
    .copy()
)
...
sample_indices = sorted(
    set(misclassified_cases_df["sample_idx"]).union(ambiguous_cases_df["sample_idx"])
)

print("\n=== Interesting local SHAP candidates ===")
print(f"Ambiguous threshold : {AMBIGUOUS_LOW:.2f} <= p(bad) <= {AMBIGUOUS_HIGH:.2f}")
print(f"Misclassified count : {int(case_report_df['is_misclassified'].sum())}")
print(f"Ambiguous count     : {int(case_report_df['is_ambiguous'].sum())}")
print(f"Selected sample idx : {sample_indices}")
